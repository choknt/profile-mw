...
// ใช้เดาชื่อโชว์จาก URL ถ้าผู้ใช้ไม่ได้กรอก displayName
        ...
        <div className="mx-auto max-w-6xl p-6">
          <section className="grid gap-8 md:grid-cols-2">
            ...
            <div className="space-y-4">
                ...
                {u.roleStaff && (
                  <span className="inline-flex items-center gap-1 rounded bg-cyan-500/20 px-2 py-0.5 text-[11px] text-cyan-300">
                    <svg viewBox="0 0 24 24" className="h-3.5 w-3.5">
                      <path d="M12 2l3 6 6 .9-4.5 4.3 1.1 6.3L12 16.9 6.4 19.5 7.5 13 3 8.9 9 8z" fill="currentColor" />
                    </svg>
                    Staff
                  </span>
                )}
              ...
              {u.roleStaff && (
                <div className="text-sm text-white/80">staff playmw.fun · support playmw.fun / modern warships game staff</div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                {(u.socials || []).map(
                  (s: { id: string; url: string; platform: string; displayName?: string }) => (
                    <a
                      key={s.id}
                      href={s.url}
                      target="_blank"
                    ...
                    >
                      {s.platform}:{" "}
                      <span className="font-medium">{s.displayName || prettyNameFromUrl(s.url)}</span>
                    </a>
                  )
                )}
              </div>
              ...
              {u.favoriteShipName && (
                <div>
                  <h3 className="mb-2 font-semibold">Favorite Ship</h3>
                  <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
                    {u.favoriteShipImage && (
                      <img src={u.favoriteShipImage} className="h-16 w-16 rounded object-cover" />
                    )}
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                />
                {u.roleLegend && (
                  <div className="absolute -left-2 -top-2 rounded bg-purple-600 px-2 py-1 text-xs">Legend</div>
                )}
                {u.roleHero && (
                  <div className="absolute -right-2 -top-2 rounded bg-orange-500 px-2 py-1 text-xs">Hero</div>
                )}
              </div>

              {(u.gallery || []).length > 0 && (
                <div>
                  <h3 className="mb-2 font-semibold">Game Gallery</h3>
                  <div className="grid grid-cols-3 gap-2">
                    {u.gallery.map((g: { id: string; url: string }) => (
