...
export default async function PublicProfile({ params }: { params: { handle: string } }) {
  ...
  const bgClass = "bg-ocean"; // คุณมีธีม ocean อยู่แล้ว
  const avatarSrc = u.avatarUrl && u.avatarUrl.trim() !== "" ? u.avatarUrl : "/avatar.png";
          ...
          <section className="grid gap-8 md:grid-cols-2">
            <div className="space-y-4">
              ...
              <div className="text-sm text-white/70">
                Game ID: <span className="font-medium">{u.gameId || "-"}</span>
              </div>

              <div className="text-sm text-white/70">
                Invite: <span className="font-medium">{u.inviteCode || "-"}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {(u.socials || []).map((s) => (
                  <a key={s.id} href={s.url} target="_blank" className="rounded bg-white/5 px-3 py-1 text-sm hover:bg-white/10">
                    {s.platform}: <span className="font-medium">{s.displayName || s.url}</span>
                  </a>
                ))}
              </div>
            ...
            <div className="space-y-4">
              <div className="relative">
                <img src={avatarSrc} className="w-full max-w-md rounded-2xl object-cover mx-auto" />
