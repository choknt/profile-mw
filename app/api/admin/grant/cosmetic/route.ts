...
export async function POST(req: Request) {
  ...
  const frameKey = String(form.get("frameKey")||"");
  const effectKey = String(form.get("effectKey")||"");
  const u = await prisma.user.findUnique({ where: { handle } });
  if (!u) return new Response("User not found", { status: 404 });

  const data:any = {};
  if (frameKey) {
    const f = await prisma.frameRef.findUnique({ where: { key: frameKey } });
    if (!f) return new Response("Frame not found", { status: 404 });
    data.frameKey = f.key;
  }
  if (effectKey) {
    const e = await prisma.effectRef.findUnique({ where: { key: effectKey } });
